package naayadaa.AlgorithmCourse.cycleDetectionDfs;

import java.util.ArrayList;
import java.util.List;

/**
 * Created by AnastasiiaDepenchuk on 27-Mar-18.
 */
public class App {

    public static void main(String[] args) {

        Vertex vertex1 = new Vertex("1");
        Vertex vertex2 = new Vertex("2");
        Vertex vertex3 = new Vertex("3");
        Vertex vertex4 = new Vertex("4");
        Vertex vertex5 = new Vertex("5");
        Vertex vertex6 = new Vertex("6");

        vertex1.addAdjacency(vertex2);
        vertex2.addAdjacency(vertex3);
        vertex3.addAdjacency(vertex2);
        vertex4.addAdjacency(vertex1);
        vertex4.addAdjacency(vertex5);
        vertex5.addAdjacency(vertex6);
       // vertex6.addAdjacency(vertex4);

        List<Vertex> vertexList = new ArrayList<>();

        vertexList.add(vertex1);
        vertexList.add(vertex2);
        vertexList.add(vertex3);
        vertexList.add(vertex4);
        vertexList.add(vertex5);
        vertexList.add(vertex6);

        CycleDetection cycleDetection = new CycleDetection();

        cycleDetection.detectCycle(vertexList);

    }
}
